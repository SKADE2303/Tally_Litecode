...
const ProblemsTable:React.FC<ProblemsTableProps> = ({ setLoadingProblems }) => {
    ...
    //defines the state of the Youtube video being rendered
    ...
    //This is to clode the Youtube popu with the close button
    const closeModal = ()=>{
        setYoutubePlayer({isOpen: false, videoId : ""});
    }

    //This is to close the Youtube popup with escape key
    useEffect(() => {
				...
				{problems.map((problem, idx) => {
					const difficulyColor =
						problem.difficulty === "Easy"
							...
							: "text-dark-pink";
					return (
						<tr className={`${idx % 2 == 1 ? "bg-blue-600" : ""}`} key={problem.id}>
							<th className='px-2 py-4 font-medium whitespace-nowrap text-dark-green-s'>
								{solvedProblems.includes(problem.id) && <BsCheckCircle fontSize={"18"} width='18' />}
							</th>
							<td className='px-6 py-4'>
								{/** If the problem exists in the thr database, take it to that problem, else link it to the Leetcode link*/}
								{problem.link ? (
									<Link
										href={problem.link}
